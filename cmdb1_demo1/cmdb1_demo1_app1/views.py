...
EnviromentTable,PortTable,LogPathTable,DeployHostTable,User,DeployLogTable,EventTable
...
def ViewDeployLog_nodeId(request,id):
    if request.method=='GET':
        ...
        for i in DeployLogTable.objects.filter(node=node):
            #这里是用node实例查出属于他的日志有哪些。
            #查出来后，需要把这些对象中log字段组合成一个数组
            #print(i.log)
...
def ViewDeployLog_event(request,id):
    ...
    logs=[]
    event=EventTable.objects.get(eventId=id)
    if sign=='getLogs':
        logIdObjects=DeployLogTable.objects.filter(event=event)
